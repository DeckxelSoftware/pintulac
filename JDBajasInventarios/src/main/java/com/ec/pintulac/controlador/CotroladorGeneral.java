...
import com.ec.pintulac.repository.VwJDBajasInventariosRepository;
...
public class CotroladorGeneral {
	...
	@Autowired
	VwJDBajasInventariosRepository vwJDVentaLogisticaFacturacionRepository;

	@Autowired
	ServicioGeneral servicioGeneral;

	@PostMapping(value = "/bajas_inventarios")
	@ApiOperation(tags = "bajas_inventarios", value = "Detallar los elementos que conforman la integración de entrada a JDE para el registro de bajas de inventarios.")
	public ResponseEntity<?> vwVentasPos() {
		try {
			Object response = "SIN DATOS";
			List<VwJdeBajasInventarios> ventas = vwJDVentaLogisticaFacturacionRepository.findAll();
			List<String> listaDatos = new ArrayList<String>();
			if (!ventas.isEmpty()) {

				for (VwJdeBajasInventarios items : ventas) {
