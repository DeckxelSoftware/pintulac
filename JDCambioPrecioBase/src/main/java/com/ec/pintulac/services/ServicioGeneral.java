...
public class ServicioGeneral {
	...
	public ConsutasPreciosItemResponse invocarJDE(ConsultasPreciosItemRequest param) {
		try {
			...
			HttpEntity<ConsultasPreciosItemRequest> requestEntity = new HttpEntity<>(param, headers);
			ResponseEntity<ConsutasPreciosItemResponse> response = restTemplate.exchange(ruta, HttpMethod.POST,
					requestEntity, ConsutasPreciosItemResponse.class);

			HttpStatus statusCode = response.getStatusCode();

			if (statusCode.is2xxSuccessful()) {
				ConsutasPreciosItemResponse consultasCostosResponse = response.getBody();
