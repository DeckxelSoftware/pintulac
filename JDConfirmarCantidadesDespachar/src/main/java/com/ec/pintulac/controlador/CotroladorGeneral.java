...
import com.ec.pintulac.repository.VwJDConfirmarCantidadesDespacharRepository;
...
public class CotroladorGeneral {
	...
	@Autowired
	VwJDConfirmarCantidadesDespacharRepository vwJDVentaLogisticaFacturacionRepository;

	@Autowired
	ServicioGeneral servicioGeneral;

	@PostMapping(value = "/confirmar_cantidades_despachar")
	@ApiOperation(tags = "confirmar_cantidades_despachar", value = "Detallar los elementos que conforman la integración de entrada a JDE para la confirmación de cantidades a despachar en bodega.")
	public ResponseEntity<?> vwVentasPos() {
		try {
			Object response = "SIN DATOS";
			List<VwJdeConfirmarCantidadesDespachar> ventas = vwJDVentaLogisticaFacturacionRepository.findAll();
			List<String> listaDatos = new ArrayList<String>();
			if (!ventas.isEmpty()) {

				for (VwJdeConfirmarCantidadesDespachar items : ventas) {
