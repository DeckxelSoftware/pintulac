...
import com.ec.pintulac.repository.VwJDGestionCargasWMSRepository;
...
public class CotroladorGeneral {
	...
	@Autowired
	VwJDGestionCargasWMSRepository vwJDVentaLogisticaFacturacionRepository;

	@Autowired
	ServicioGeneral servicioGeneral;

	@PostMapping(value = "/gestion_Cargas_wms")
	@ApiOperation(tags = "gestion_Cargas_wms", value = "Detallar integración de ingreso a envíos a cargas.")
	public ResponseEntity<?> vwVentasPos() {
		try {
			Object response = "SIN DATOS";
			List<VwJdeGestionCargasWMS> ventas = vwJDVentaLogisticaFacturacionRepository.findAll();
			List<String> listaDatos = new ArrayList<String>();
			if (!ventas.isEmpty()) {

				for (VwJdeGestionCargasWMS items : ventas) {
