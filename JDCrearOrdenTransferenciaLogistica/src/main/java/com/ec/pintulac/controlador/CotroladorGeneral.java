...
import com.ec.pintulac.repository.VwJDJDCrearOrdenTransferenciaLogisticaRepository;
...
public class CotroladorGeneral {
	...
	@Autowired
	VwJDJDCrearOrdenTransferenciaLogisticaRepository vwJDVentaLogisticaFacturacionRepository;

	@Autowired
	ServicioGeneral servicioGeneral;

	@PostMapping(value = "/crear_orden_transferencia_logistica")
	@ApiOperation(tags = "crear_orden_transferencia_logistica", value = "Detallar los elementos que conforman la integración de entrada a JDE para la creación de órdenes de transferencia logística.")
	public ResponseEntity<?> vwVentasPos() {
		try {
			Object response = "SIN DATOS";
			List<VwJdeCrearOrdenTransferenciaLogistica> ventas = vwJDVentaLogisticaFacturacionRepository.findAll();
			List<String> listaDatos = new ArrayList<String>();
			if (!ventas.isEmpty()) {

				for (VwJdeCrearOrdenTransferenciaLogistica items : ventas) {
