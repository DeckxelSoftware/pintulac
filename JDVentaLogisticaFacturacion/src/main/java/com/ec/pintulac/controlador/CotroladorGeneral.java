...
import com.ec.pintulac.repository.VwJDVentaLogisticaFacturacionRepository;
...
public class CotroladorGeneral {
	...
	@Autowired
	VwJDVentaLogisticaFacturacionRepository vwJDVentaLogisticaFacturacionRepository;

	@Autowired
	ServicioGeneral servicioGeneral;

	@PostMapping(value = "/venta_logistica_Facturacion")
	@ApiOperation(tags = "venta_logistica_Facturacion", value = "Detallar integración para pedidos de venta cuando el proceso logístico y la facturación suceden en JD Edwards. ")
	public ResponseEntity<?> vwVentasPos() {
		try {
			Object response = "SIN DATOS";
			List<VwJdeVentaLogisticaFacturacion> ventas = vwJDVentaLogisticaFacturacionRepository.findAll();
			List<String> listaDatos = new ArrayList<String>();
			if (!ventas.isEmpty()) {
				for (VwJdeVentaLogisticaFacturacion items : ventas) {
