...
import com.ec.pintulac.repository.VwJDNotasCreditoDebitoPosRepository;
...
public class CotroladorGeneral {
	...
	@Autowired
	VwJDNotasCreditoDebitoPosRepository vwJDVentaLogisticaFacturacionRepository;

	@Autowired
	ServicioGeneral servicioGeneral;

	@PostMapping(value = "/notas_credito_debito_pos")
	@ApiOperation(tags = "notas_credito_debito_pos", value = "Detallar las integraciones que componen la interfaz de entrada para notas de crédito y notas de débito que vienen facturados desde el sistema POS, donde en JDE se generará baja de inventario y afectación contable. ")
	public ResponseEntity<?> vwVentasPos() {
		try {
			Object response = "SIN DATOS";
			List<VwJdeNotasCreditoDebitoPos> ventas = vwJDVentaLogisticaFacturacionRepository.findAll();
			List<String> listaDatos = new ArrayList<String>();
			if (!ventas.isEmpty()) {

				for (VwJdeNotasCreditoDebitoPos items : ventas) {
