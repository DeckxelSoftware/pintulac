...
import com.ec.pintulac.repository.VwJDJDDespachoPedidosRepository;
...
public class CotroladorGeneral {
	...
	@Autowired
	VwJDJDDespachoPedidosRepository vwJDVentaLogisticaFacturacionRepository;

	@Autowired
	ServicioGeneral servicioGeneral;

	@PostMapping(value = "/despacho_pedidos")
	@ApiOperation(tags = "despacho_pedidos", value = "Detallar integración para pedidos de venta cuando el proceso logístico y la facturación suceden en JD Edwards. ")
	public ResponseEntity<?> vwVentasPos() {
		try {
			Object response = "SIN DATOS";
			List<VwJdeJDDespachoPedidos> ventas = vwJDVentaLogisticaFacturacionRepository.findAll();
			List<String> listaDatos = new ArrayList<String>();
			if (!ventas.isEmpty()) {

				for (VwJdeJDDespachoPedidos items : ventas) {
