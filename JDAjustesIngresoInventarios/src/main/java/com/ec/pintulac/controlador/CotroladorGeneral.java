...
import com.ec.pintulac.repository.VwJDAjustesIngresoInventariosRepository;
...
public class CotroladorGeneral {
	...
	@Autowired
	VwJDAjustesIngresoInventariosRepository vwJDVentaLogisticaFacturacionRepository;

	@Autowired
	ServicioGeneral servicioGeneral;

	@PostMapping(value = "/ajustes_ingreso_inventarios")
	@ApiOperation(tags = "ajustes_ingreso_inventarios", value = "Detallar los elementos que conforman la integración de entrada a JDE para el registro de ajustes de ingreso en inventarios.")
	public ResponseEntity<?> vwVentasPos() {
		try {
			Object response = "SIN DATOS";
			List<VwJdeAjustesIngresoInventarios> ventas = vwJDVentaLogisticaFacturacionRepository.findAll();
			List<String> listaDatos = new ArrayList<String>();
			if (!ventas.isEmpty()) {

				for (VwJdeAjustesIngresoInventarios items : ventas) {
