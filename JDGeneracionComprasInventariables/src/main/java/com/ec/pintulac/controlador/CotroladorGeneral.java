...
import com.ec.pintulac.repository.VwJDGeneracionComprasInventariablesRepository;
...
public class CotroladorGeneral {
	...
	@Autowired
	VwJDGeneracionComprasInventariablesRepository vwJDVentaLogisticaFacturacionRepository;

	@Autowired
	ServicioGeneral servicioGeneral;

	@PostMapping(value = "/generacion_compras_inventariables")
	@ApiOperation(tags = "generacion_compras_inventariables", value = "Detallar los elementos que conforman la integración de entrada a JDE para el registro de órdenes de compra local (OL) que van a venir desde un sistema satélite.")
	public ResponseEntity<?> vwVentasPos() {
		try {
			Object response = "SIN DATOS";
			List<VwJdeGeneracionComprasInventariables> ventas = vwJDVentaLogisticaFacturacionRepository.findAll();
			List<String> listaDatos = new ArrayList<String>();
			if (!ventas.isEmpty()) {

				for (VwJdeGeneracionComprasInventariables items : ventas) {
